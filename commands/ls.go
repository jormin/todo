...
func List(ctx *cli.Context) error {
	...
	headers := []interface{}{"#", "ID", "Date", "Content", "Level", "Status", "UpdateTime"}

	w := tabwriter.NewWriter(os.Stdout, 10, 0, 5, ' ', tabwriter.StripEscape)
	...
	for index, item := range todos {
		...
		str := fmt.Sprintf(
			contentFormat, index+1, item.ID, item.Date, item.Content, entity.TodoLevelTexts[item.Level],
